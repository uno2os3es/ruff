...
dedicated `__get__` method) are _not_ bound-method descriptors. If accessed as class attributes via
...
However, this leads to unsoundness: `C3().callable_m` is actually `C3.method` which _is_ a
...
decorator syntax. This is inconsistent, because the above _should_ be equivalent to the following,
...
The reason for this is that the heuristic is problematic. We don't _know_ that the `Callable` in the
