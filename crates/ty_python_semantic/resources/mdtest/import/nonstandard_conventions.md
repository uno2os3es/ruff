...
- **first from first serve**: only the _first_ `from..import` in an `__init__.py(i)` that imports a
...
- **dot re-exports**: `from . import a` in an `__init__.pyi` is considered a re-export of `a`
  ...
  `from .a import b` and equivalent does _not_ introduce a re-export.

Note: almost all tests in here have a stub and non-stub version, because we're interested in both
defining symbols _at all_ and re-exporting them.
